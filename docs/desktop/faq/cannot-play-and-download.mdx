...
尝试更换网络，如切换到手机数据流量。若数据流量还是不行，则尝试开关一下手机的「飞行模式」后再试。

若使用家庭网络，可尝试将光猫断电 5 分钟左右，再通电联网并播放。
...
### Windows 平台上，所有歌曲都提示 `音频加载错误，5 秒后切换下一首`

按照以下方法解决。先尝试第一个，若无效则尝试第二个：

- 尝试关闭「Internet 选项」的代理设置。
- 卸载软件（**注意备份歌单**），安装时选择「**仅为我安装**」，重新安装到系统分区（使用默认安装路径即可）。

:::tip
如果你不知道怎么关闭「Internet 选项」的代理设置，可以按以下步骤操作：

- 按 <kbd>Windows</kbd>+<kbd>R</kbd> 键打开「运行」窗口，输入 `inetcpl.cpl` 后回车。
- 在打开的「Internet 选项」对话框中，依次点击「连接 → 局域网设置」。
- 在弹出的新窗口中，把「代理服务器」下方的勾去掉。
- 如果「自动配置」下的勾也有被勾选，建议也一并去掉。
- 最后点击「确定」关闭所有弹窗，重启 LX Music。
...
更换歌曲下载路径即可解决（该问题通常是由于设置的下载目录没有读写权限导致的）。
