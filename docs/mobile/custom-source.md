...
- `@description `：源的描述，建议不要过长，36 个字符以内，可不填，不填时可以删除 `@description`。
...
- 移动版只有极少部分宿主环境API可用，详情看「可用宿主环境 API」说明。
...
以上是自定义源功能在移动版与桌面版的区别，目前仅提供以上工具方法及宿主 API，如果需要其他方法可以在 GitHub [提交 Issue](https://github.com/lyswhut/lx-music-mobile/issues?q=is%3Aissue+) 讨论。
